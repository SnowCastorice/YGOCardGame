...
### 开发者工具（`js/game.js`）
- 内网环境（localhost）自动解锁隐藏功能区域，无需点击标题 5 次
- "强制使用 R2 图源"开关仅在内网环境显示（外网本身走 R2，开关无意义）

## v1.11.3（2026-04-15）— 背包卡图修复 + 大图预览显示编号
...
- 全局添加 `touch-action: manipulation`，禁止浏览器将快速连点解释为双击缩放手势
- 保留捏合缩放能力，不影响无障碍访问

## v1.8.2（2026-03-28）— 图鉴代码重构 + 后台统计优化

### 图鉴展开逻辑重构（`js/game.js`）
- 去除 `isLochSpecial` 变量和条件判断，展开逻辑改为无条件执行（所有卡包默认展开）
- 注释从"LOCH 卡包图鉴特殊处理"更新为"图鉴默认展开模式"

### 后台统计页面优化（`admin/stats.html`）
- 数字格式化支持亿级（≥1亿显示为"X.XX亿"）
- 新增排序功能：按发售时间（默认，新→旧）或按开包数（多→少）
- 卡包缩写后显示发售日期
